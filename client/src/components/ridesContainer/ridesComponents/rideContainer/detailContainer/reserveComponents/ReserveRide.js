...
export default class ReserveRide extends React.Component {
  ...
  handleReserveRide(note) {
    ...
    if(newRide.passengersRequested==undefined){
      newRide.passengersRequested=[];
    }
    if(newRide.requestStatus==undefined){
      newRide.requestStatus = {};
    }

    newRide.requestStatus[this.state.user._id] = "Pending";
    console.log(newRide);
    newRide.passengersRequested.push(this.state.user);
