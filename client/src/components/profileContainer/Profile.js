...
import ProfileRequests from './profileComponents/ProfileRequests'
...
export default class Profile extends React.Component {
  ...
  render() {
    ...
    if (!this.state.user) {
      ...
      listRides = this.state.rides.filter(ride =>
        (this.state.user._id in ride.requestStatus)).map((ride) => {
        if (ride.type === 'offer') {
        body =  [body,<div style={{padding:"15px"}}></div>,(<ProfileRequests
              rides = {ride}
              type = "request"
              user = {this.state.user}
              status = ""
