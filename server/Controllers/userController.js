...
const transporter = nodemailer.createTransport({
  ...
  domain: 'gmail.com',
  auth: {
    user: "bharathkusuluru@gmail.com",
    pass: "jilylvklrrjctldk",
  }
});
...
const AccountVerificationEmail = (user) => {
  ...
  console.log("The verification mail is: "+link);
  const mailOptions = {
    from: '"VT Carpool Admin" <bharathkusuluru@gmail.com>',
    to: user.email,
    subject: 'VT Carpool - Your account registration',
...
const verificationData = (user) => {
    console.log("Found user id as: "+ user._id);
    const baseurl = process.env.NODE_ENV === 'production' ? process.env.HEROKU_SERVER : 'http://localhost:3000';
    User.findByIdAndUpdate(user._id, { isVerified: true }, (error) => {
      if (error) {
  ...
  async.waterfall([
    ...
    (token, user, done) => {
      const mailOptions = {
        to: user.email,
        from: '"VT Carpool Admin" <bharathkusuluru@gmail.com>',
        subject: '✔ VT Carpool - Reset your password',
  ...
  async.waterfall([
    ...
    (user, done) => {
      const mailOptions = {
        from: '"VT Carpool Admin" <bharathkusuluru@gmail.com>',
        to: user.email,
        subject: 'VT Carpool - Your password has been changed',
        html: `${'Hello,\n\n' +
